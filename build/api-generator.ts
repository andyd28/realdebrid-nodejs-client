...
import { Method, MethodParameter } from "./api-types";
import fs from "fs";
...
const createInterfaceName = (methodName: string, suffix: string) => toPascalCase("RealDebrid", methodName, suffix);
...
const data: Record<string, Method> = JSON.parse(output);
...
const classOptionalOptions: string[] = ["access_token"];
...
const propsToString = (obj: Record<string, any> | Array<Record<string, any>>, depth: number): string => {
    ...
    for (let [prop, value] of Object.entries(obj)) {
        ...
        if (prop === "int") prop = "number";
        if (prop === "string" || prop === "int") prop = `[key: ${[prop]}]`;
 ...
 */
const addRequestInterface = (methodName: string, method: Method) => {
 ...
 */
const addResponseInterface = (methodName: string, method: Method): string => {
 ...
 */
const getParameterDeclaration = (methodName: string, method: Method): string => {
 ...
 */
const getResponseDeclaration = (methodName: string, method: Method): string => {
    let declaration = "any";
